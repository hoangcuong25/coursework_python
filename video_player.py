...
from create_video_list import Create_video_list
...
def create_video_list_clicked():
    ...
    Create_video_list(tk.Toplevel(window))

def update_Video_clicked():
    status_lbl.configure(text="Create Video List button was clicked!")
    Update_video(tk.Toplevel(window))
...
check_videos_btn = tk.Button(window, text="Check Videos", command=check_videos_clicked, fg= 'blue')
check_videos_btn.grid(row=1, column=0, padx=10, pady=10)

create_video_list_btn = tk.Button(window, text="Create Video List", command=create_video_list_clicked, fg= 'blue')
create_video_list_btn.grid(row=1, column=1, padx=10, pady=10)

update_videos_btn = tk.Button(window, text="Update Videos", command=update_Video_clicked, fg= 'blue')
